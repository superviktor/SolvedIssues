...
{
    public class AzureTableRepository<T> : IAzureTableRepository<T> where T : class, ITableEntity, new()
    {
        ...
        /// <returns></returns>
        public async Task<T> GetAsync(string tableName, string partitionKey, string rowKey)
        {
        ...
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public async Task<IEnumerable<T>> QueryAsync(string tableName, TableQuery<T> query)
        {
            var table = await EnsureTable(tableName).ConfigureAwait(false);
        ...
        /// <returns></returns>
        public async Task<IEnumerable<T>> AddBatchAsync(string tableName, IEnumerable<ITableEntity> entities, BatchOperationOptions options = null)
        {
            var table = await EnsureTable(tableName).ConfigureAwait(false);
            ...
            while (entitiesOffset < entities?.Count())
            {
                ...
                tasks.AddRange((IEnumerable<Task<IList<TableResult>>>)table.ExecuteBatchAsync(batchOperation));
            }
        ...
        /// <returns></returns>
        private async Task<IEnumerable<T>> QueryAsync(CloudTable table, TableQuery<T> query)
        {
            var entities = new List<T>();
        ...
        /// <param name="table"></param>
        /// <returns></returns>
        private async Task<IEnumerable<T>> QueryAsyncWithTakeCount(CloudTable table, TableQuery<T> query)
        {
            var entities = new List<T>();
